...
function LedgerContent() {
    ...
    const fetchLedger = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/pharmacist/vendor/${selectedId}/ledger`,
          { headers: { Authorization: `Bearer ${user.access_token}` } }
        );
        const json = await res.json();
        setData(json);
  ...
  const exportPdf = useCallback(async () => {
    ...
    try {
      ...
      doc.text("Vendor account — balance sheet style statement", margin, y);
      y += 22;
      doc.setFontSize(11);
      doc.setTextColor(60, 60, 60);
      doc.text(`Vendor: ${data.vendor.name}`, margin, y);
      y += 16;
      ...
      doc.text(`Closing / outstanding balance: ${formatCurrency(data.vendor.currentBalance)}`, margin, y);
      ...
      const safeName = data.vendor.name.replace(/[^\w\s-]/g, "").slice(0, 40) || "ledger";
      doc.save(`vendor-ledger-${safeName}.pdf`);
    } catch (e) {
      console.error(e);
    } finally {
              ...
              <div className="w-full sm:w-80">
                ...
                <Select value={selectedId} onValueChange={setSelectedId}>
                  <SelectTrigger className="h-11 border-gray-200 focus:ring-red-500/20">
                    <SelectValue placeholder={loadingVendors ? "Loading..." : "Choose a vendor..."} />
                  </SelectTrigger>
                  <SelectContent>
                    {vendors.map((m) => (
              ...
              {data && (
                <div className="flex-1 flex justify-end">
                  <div className="px-6 py-4 bg-red-50 rounded-2xl border border-red-100 min-w-[200px]">
                    ...
                    <p className="text-3xl font-black text-red-900">{formatCurrency(data.vendor.currentBalance)}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <AnimatePresence mode="wait">
          {loadingLedger ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                    ...
                    <div className="p-5 bg-red-50/40">
                      ...
                      <p className="text-xl font-bold text-red-900 mt-1">
                        {formatCurrency(data.vendor.currentBalance)}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className="overflow-hidden border border-gray-100 rounded-xl shadow-sm bg-white">
                <div className="border-l-4 border-l-red-500 bg-red-50/30 px-5 py-4">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    {data.vendor.name}: line items from purchase invoices; payment method on supplier
                    payments only.
                  </p>
                </div>
            ...
            <div className="py-20 text-center bg-white rounded-xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-500 font-medium">Please select a vendor to view their ledger.</p>
            </div>
          ) : null}
        </AnimatePresence>
      </div>
